...
from util.utils_webarena import fetch_browser_info, fetch_page_accessibility_tree,\
...
def extract_information(text):
    ...
    for key, pattern in patterns.items():
        ...
        if match:
            if key in ["click", "wait", "goback", "google", "up", "down", "left", "right"]:
                # no content
...
def summarize_results(messages):
    assistant_msg = [msg for msg in messages if msg['role'] == 'assistant']
    # overall_score = int(assistant_msg[0]['content'].split('Overall Score: ')[1].split('\n')[0].strip())

    score_list = []
    for msg in assistant_msg[3:]:
        try:
            current_score = msg['content'].split('Score: ')[1].split('\n')[0].strip()
            # print(current_score)
        except:
            current_score = 'NaN'
    ...
    agent_avg_score = sum(score_list) / len(score_list) if len(score_list) > 0 else 0
    ...
    print(score_list)
    return agent_avg_score, score_list
